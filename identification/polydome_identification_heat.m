...
load('exp4')
load('exp5')
load('exp1_15min_heat')
load('exp2_15min_heat')
...
%%
% data_iden = merge(exp1, exp2, exp3, exp4, exp5);
data_iden = merge(exp7);
data_val = merge(exp1, exp2, exp3, exp4, exp5, exp6);
% data_val = merge(exp6);
...
pred_step = 10;
m1 = arx(data_iden_de,[6 [6 1 5] [1 1 1]]);
...
pred_step = 10;
m3 = arx(data_iden_real,[1 [6 1 5] [1 1 1]]);
...
% save('./model/model_ss3','m1')
%% 
% Plot: prediction vs measured
% 'Data_val_de' contains several datasets
pred_step = 86;
figure
compare(data_iden_de(:,:,:,1),m1,m2,m3,pred_step)
figure
compare(data_iden_de(:,:,:,2),m1,m2,m3,pred_step)
%%
% figure
% resid(data_val_de, m1)
% Time validation: simulation
num = 1;
figure
compare(data_val_de(:,:,:,num), m1,m2,m3, 24);
figure
compare(data_val_de(:,:,:,num), m1,m2,m3,96);
figure
compare(data_val_de(1:96,:,:,num), m1,m2,m3);
%% Correlation Test
