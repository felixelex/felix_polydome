...
%         Q; % Time-varying penalty for y
    ...
    methods
        function obj = OptDeePCRobust(sys, N_ini, N_pred)
            ...
            obj.u_bound = sdpvar(sys.nu*N_pred,1,'full');
            obj.y_norm = [];
...
%             if soft == 1
%                 obj.slack_pred = 100*ones(size(F_y,1),1) ;%sdpvar(size(F_y,1),1,'full');
%                 obj.Constraints = [obj.Constraints;lambda_y'*f_w<=obj.f_y + obj.slack_pred -F_y*obj.y_norm];
%             else
...
%             opts = sdpsettings('solver','gurobi','verbose',1);
        ...
        function add_constraint_du(obj,Hankel_p,Hankel_f,w_g_tilde)
            nu = obj.sys.nu; nw = obj.sys.nw; ny = obj.sys.ny;
            ...
            K_tem2 = [zeros(nw*obj.N_pred,obj.sys.nu*obj.N_pred),eye(nw*obj.N_pred);
                eye(obj.sys.nu*obj.N_pred)+obj.Ku,obj.K]; % u = u_bar + Kdw + du + obj.Kudu = u_bar+ K_new*dw_new
            
            % robust constraints, each column corresponds to one robust constraint
            % robust input cons
            lambda_u = sdpvar(size(F_w,1),size(F_u,1),'full');
                      
            obj.slack_pred = sdpvar(size(F_u,1),1,'full'); % Add soft variables
            obj.Constraints = [obj.Constraints;lambda_u'*f_w<=f_u + obj.slack_pred  - F_u*obj.u_norm];
