...
%             obj.Q = sdpvar(sys.ny*N_pred,sys.ny*N_pred,'full');
        ...
        function add_adaptive_KKT(obj, Hankel_p, Hankel_f, Qg,w_s)
...
%             opts = sdpsettings('solver','gurobi','verbose',1);
        ...
        function [u_opt_seq, y_opt_seq,g_opt] = solve(obj)
            u_opt_seq = [];
            y_opt_seq = [];
            options = sdpsettings('verbose',1,'solver','quadprog','gurobi.TimeLimit', 2); % gurobi
...
%             slack_opt = double(obj.slack);
            ...
            'optimal cost - cost of slack variables'
            cost_opt - double(100*norm(obj.slack,1))%double(100* obj.slack'*obj.slack) 
            g_opt = double(obj.g);
        end 
