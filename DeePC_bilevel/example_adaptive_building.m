...
A = @(t) A_tem + 0.05*sin(2*pi*t/T_day)*eye(length(A_tem));
B = [0.07;0.006;0.004];
...
[w_pred, w_real,w_y,w_rand,W] = get_ini_dist(T_day,num_day,num_T, 0.0);
[w_pred, w_real,w_y] = get_dist(w_pred, w_rand, w_y,num_day,num_T, T_day, 0.0);
%%
% Define the object of the time-varying linear system
mysys = LinearSystemNoiseTV(A, B, C, Q, R, U, Y,W);
...
% Current time
t = T_start;
controller.initialize_deepc_noise_TV(t);
...
% Run the controller
        ...
        % (2) Use L2-norm for g 
        if num_which == 1
            controller.set_deepc_L2_noise(Hankel_p, Hankel_f, Qg, w_s)
        % (3) Use bi-level
        elseif num_which == 2
            controller.set_deepc_adaptive_noise(Hankel_p, Hankel_f, Qg, w_s)
        end

            [u_opt, u_seq, y_seq] = controller.solve_deepc(t, u_ini, w_ini, y_ini);
