...
A = @(t) A_tem %+ sin(2*pi*t/T_day)*diag([0.005, 0.005,0.005]);
...
set(h,'fontsize',24, 'interpreter', 'latex')
title('Simulation: no robust','fontsize',20)
...
function [w_pred, w_real,w_y,w_rand, W] = get_ini_dist(T_day,num_day,num_T, var_y)
    Hw = [eye(2);-eye(2)];
    bw = [1.5;0.05;1.5;0.05];
    W = Polyhedron('A', Hw, 'b', bw);

    w1_rand = -1 + 2*rand(1,T_day*num_day);
    w2_rand = -0.05 + 0.1*rand(1,T_day*num_day);
    ...
    w2_pred = zeros(1,T_day*num_day);
        ...
        for j = 0:num_T-1
            ...
            w2_rand(j*T_day+1:T_day*(j+1)) = -0.05 + 0.1*rand(1,T_day);
